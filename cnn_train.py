...
def train(x_train, y_train):
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            # if pointer == (len(x_train) // batch_size):
            #     pointer = 0
            xs, ys = get_batch(x_train, y_train, batch_size=batch_size)
            # xs, ys = get_next_batch(X_train, y_train, batch_size=batch_size)
            # ys = tf.reshape(tf.one_hot(ys, depth=5), [batch_size, OUTPUT_NODE])
            ...
            if step % 2000 == 0:
                print("After %d training step(s), loss on training batch is %g，"
                      "train accuracy = %f." % (step, loss_value,accuracy_score))
                saver.save(sess, os.path.join(MODEL_SAVE_PATH, MODEL_NAME), global_step=global_step)

def guess(x_test):
    with tf.Graph().as_default() as graph: # 设置默认graph
        # 定义输入输出格式
        ...
        with tf.Session() as sess:
            ...
            if ckpt and ckpt.model_checkpoint_path:
                ...
                # print(ys.shape)
                label = sess.run(y_, feed_dict={x: x_test.reshape(1, x_test.shape[0],
